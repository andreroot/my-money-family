def my_destin_pushout_csv(df, path_csv):
    ...
    for index_dp, row_dp in df_type.iterrows():
        df["tipo_custo"] = df.apply(lambda x: row_dp['valor']  if x["descricao"]==row_dp['de_para'] else
                                                                ( "sabesp" if row_cc["descricao"].find('SABESP')>=0 else 
                                                                ( "financiamento" if row_cc["descricao"].find('IMOBILIARIO')>=0 else "compras")))
