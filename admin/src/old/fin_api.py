...
def consulta_excel(path, file_excel, tipo, mes_ano):
    ...
    if tipo=='debito':
        ...
        print(f'{path}/data/excel/{file_excel}.xls')

        if os.path.exists(f'{path}/data/excel/{file_excel}.xls'):
            print('\n')
...
def fake(df, dt, tipo_custo, custo, valor_custo):
    #dfc.columns
    ...
    # custo=['DEB AUTOR UNICEF','DA  CLARO MOVEL 66459258','DA  NET SERVICOS 5680788','FINANC IMOBILIARIO','PIX ELIZABTEH','PIX QRS CEA PAY14/05']
    # valor_custo=[85.00, 149.00, 225.59, 2789.42, 1400, 364.28]
    ...
    # 17/06/2024	DA  CLARO MOVEL 66459258		-149,90
    # 17/06/2024	DA  ELETROPAULO 11701161		-126,41
    # 20/06/2024	DA  NET SERVICOS 5680788		-225,59
    # 24/06/2024	FINANC IMOBILIARIO   027		-2.789,42
    # 24/06/2024	FINANC IMOBILIARIO   027		-2.789,42
    # 24/06/2024	DA  COMGAS 51448254		-455,28

    #net	DA NET SERVICOS 5680788	225.59	2024-06-01	2024-06-05
    #cea	PIX QRS CEA PAY14/05	364.28	2024-06-01	2024-06-05
    #igreja	PIX TRANSF IGREJA 10/05	100.00	2024-06-01	2024-06-05

    # df = pd.concat([df, df_fake], ignore_index=True)

    # df.loc[(df['classificacao_custo']=='fixo') & (pd.to_datetime(df['dt_mes_base'])==dt) \
    #                 & ((pd.to_datetime(df['dt_custo'])>='2024-06-05')&(pd.to_datetime(df['dt_custo'])<'2024-06-15'))]
