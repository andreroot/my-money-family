...
def construcao_tipo_custo(df):
    ...
    lista_tp=['#2024_2023','igreja','boleto','banco','claro','ajuda','emprestimo','cartao','net','financiamento','gas','luz','Taxa_banco','escola_futebol','juros','celular','transporte','transporte_pub','#2022','enel','carro','saque','comgas','oferta','sabesp','doação','aplicacao','seminario','cartão','creche','poupança','psicologa','ceforte','sptransp','escola']
...
def process(query, flag_fake=False):
    ...
    df_tp[~df_tp['tipo_custo_alt'].isin(['financiamento','emprestimo','boleto','cartao','juros','banco','Taxa_banco','saque',
                                        ...
                                        '#','alimentacao','feira','mercado','padaria','transporte','transporte_pub','recargapay','cea','ifood','cabelereiro','cabeleireiro','farmacia','roupa','igreja','ajuda','pernambucanas','dora',
...
def analytics_percent(df, query):
    ...
    def get_valor_custo(df, tp, dt):
        return df[["valor_custo"]][ (df['classificacao_custo']=='fixo') & (df['tipo_custo_alt']==tp)&(pd.to_datetime(df['dt_mes_base'])==dt)].sum().values.tolist()
    ...
    for idx, rw in enumerate(list_tp):
        ...
        valor_med_anual =  float(valor_sum_anual[0]/12)
...
def analytics_variado_percent(query):
    ...
    def get_valor_custo(df, tp, dt):
        #return df[["valor_custo"]][ (df['classificacao_custo']=='fixo') & (df['tipo_custo_alt']==tp)&(pd.to_datetime(df['dt_mes_base'])==dt)].sum().values.tolist()
        return df[["valor_custo"]][ (df['classificacao_custo']=='variado') & (df['tipo_custo_alt']==tp)&(pd.to_datetime(df['dt_mes_base'])==dt)].sum().values.tolist() #.sort_values("valor_custo", ascending=False)
    ...
    for idx, rw in enumerate(list_tp):
        ...
        #valor_jan   = get_valor_custo(df, rw[0], dt.date.strftime(list_dt[0][0],"%Y-%m-%d"))   #df[["valor_custo"]][ (df['classificacao_custo']=='variado') & (df['tipo_custo_alt']==rw[0])&(pd.to_datetime(df['dt_mes_base'])=='2024-01-01')].sum().values.tolist() #.sort_values("valor_custo", ascending=False)
        valor_jan   = get_valor_custo(df, rw[0], dt.date.strftime(list_dt[0][0],"%Y-%m-%d")) #.sort_values("valor_custo", ascending=False)
        valor_fev   = get_valor_custo(df, rw[0], dt.date.strftime(list_dt[1][0],"%Y-%m-%d")) #.sort_values("valor_custo", ascending=False) df[["valor_custo"]][ (df['classificacao_custo']=='fixo') & (df['tipo_custo_alt']==rw[0])&(pd.to_datetime(df['dt_mes_base'])=='2024-02-01')].sum().values.tolist() #.sort_values("valor_custo", ascending=False)
        valor_mar   = get_valor_custo(df, rw[0], dt.date.strftime(list_dt[2][0],"%Y-%m-%d")) #.sort_values("valor_custo", ascending=False) df[["valor_custo"]][ (df['classificacao_custo']=='fixo') & (df['tipo_custo_alt']==rw[0])&(pd.to_datetime(df['dt_mes_base'])=='2024-03-01')].sum().values.tolist() #.sort_values("valor_custo", ascending=False)
        valor_abr   = get_valor_custo(df, rw[0], dt.date.strftime(list_dt[3][0],"%Y-%m-%d")) #.sort_values("valor_custo", ascending=False) df[["valor_custo"]][ (df['classificacao_custo']=='fixo') & (df['tipo_custo_alt']==rw[0])&(pd.to_datetime(df['dt_mes_base'])=='2024-04-01')].sum().values.tolist() #.sort_values("valor_custo", ascending=False)
