...
def generate_data_from_sheets(sheet_name="extrato_2025", tab_name="extract_cust_transform"):
    ...
    print(f"TOTAL DE LINHAS EXTRACT: {len(df)} / FILE: {tab_name}", "\n")

    return df
    
def generate_analytics_sheets(df, sheets="resumo financeiro", tab="previsao_plano_financeiro"):
    # Abra a planilha pelo nome
...
def generate_analytics_sheets_param(df, sheets="resumo financeiro", tab="custo_extract_all"):
    # Abra a planilha pelo nome
    sh = connect().open(sheets)
