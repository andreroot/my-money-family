...
def func_generate_type_cust(df, ano):


    def func_depara_sheets(df):
        from getdata.get_sheets import get_depara_tipo_custo
        print(f"ETAPA 1.1 - SHEETS - LER DF TOTAL: {len(df)}", "\n")
        # --> list_depara_legado
        #--> df original aplicar depara csv - sheets

        list_depara_sheets = get_depara_tipo_custo()

        df["tipo_custo"] = df.apply(lambda x: next((y[1] for y in list_depara_sheets if y[0] in x["descricao"]), None), axis=1)

        print(f"ETAPA 1.2 - SHEETS -  LER DF APOS CATEGORIZAR TIPO DE CUSTO - TOTAL DE NULO: {len(df[df['tipo_custo'].isnull()])}", "\n")

        return df
        

    def func_legado(df, ano):
        from getdata.get_parquet import get_parquet_silver
        from generate.generate_parquet import generate_parquet_categoria

        df1 = df[df['tipo_custo'].isnull()].copy()
        print(f"ETAPA 2.1 - PARQUET|CUSTO -  LER DF TOTAL:  {len(df1)}", "\n")
        # --> list_depara_csv
        # Filtrar apenas linhas onde tipo_custo está nulo

        # --> LISTAGEM DE TIPOS CATEGORIZADOS NO PARQUET CUSTO(extract_cust_transform_ano)
        dfl = get_parquet_silver(f'extract_cust_transform_{ano}','analytics_pushout_saida')
        dfl[['descricao','tipo_custo']].to_json("./output/depara_legado.json", orient="records", force_ascii=False, indent=2)   
        
        # gerado listagem de categorias e aplicar mno dataframe recebeido
        list_depara_legado = dfl[['descricao','tipo_custo']].values.tolist()
        #zerar dataframe para descarregar memoria
        dfl = pd.DataFrame()

        df1["tipo_custo"] = df1.apply(lambda x: next((y[1] for y in list_depara_legado if y[0] in x["descricao"]), None), axis=1)

        print(f"ETAPA 2.2 - PARQUET|CUSTO - LER DF APOS CATEGORIZAR TIPO DE CUSTO - TOTAL DE NULO: {len(df1[df1['tipo_custo'].isnull()])}", "\n")

        return df1


    def func_diacono_ia(df):
        ...
        # Filtrar apenas linhas onde tipo_custo está nulo
        df2 = df[df['tipo_custo'].isnull()].copy()
        print(f"ETAPA 3.1 - IA - LER DF TOTAL: {len(df2)}", "\n")
        # --> gerar lista de descrições nulas apartir do dataframe 
        lista_descricao_df = df2["descricao"].drop_duplicates().values.tolist()

        categoria=[classificar_com_ia(l) for l in lista_descricao_df]

        # gerado uma listagem de pares de tipo de custo e categoria
        list_diacono_ia = [[ item['original'], item['categoria']] for item in categoria]

        df2["tipo_custo"] = df2.apply(lambda x: next((y[1] for y in list_diacono_ia if y[0] in x["descricao"]), None), axis=1)

        print(f"ETAPA 3.2 - IA - LER DF APOS CATEGORIZAR TIPO DE CUSTO - TOTAL DE NULO: {len(df2[df2['tipo_custo'].isnull()])}", "\n")
    ...
    dfd  = pd.DataFrame()
    dfd = func_depara_sheets(df)
    
    # dfd_2 = func_legado(dfd, ano)
    # dfd_2[~dfd_2['tipo_custo'].isnull()]

    dfd_3 = func_diacono_ia(dfd)

    df = pd.DataFrame()
    df = pd.concat([dfd[~dfd['tipo_custo'].isnull()],dfd_3[~dfd_3['tipo_custo'].isnull()]],ignore_index=True)
