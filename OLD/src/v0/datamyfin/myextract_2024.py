...
# insert table
from insertmyfin.mygcptablefinfam import insert_df_pushout
from insertmyfin.mygcptablefinfam import insert_df_stop
from insertmyfin.mygcptablefinfam import insert_df_pulling
...
def my_extract_excel_deb(path, file):
    ...
    path_excel_file_deb = f'{path}\\v0\\data\\excel\\{file}'
...
def my_type_modal_finance(df, path, file, tipo_doc_extract, tab_ano ):
    ...
    if len(df):
        ...
        if tipo_doc_extract=='pulling':
            ...
            insert_df_pulling(my_pulling_money(df), f'{path}\\v0\\json\\pulling.json',f'devsamelo2.dev_domestico.recebido_{tab_ano}_excel')
        ...
        #
            ...
            insert_df_pushout(my_pushout_money(path, df), f'{path}\\v0\\json\\pushout.json',f'devsamelo2.dev_domestico.custo_{tab_ano}_excel')
        ...
        #
            ...
            insert_df_stop(my_stop_money(df), f'{path}\\v0\\json\\stop.json',f'devsamelo2.dev_domestico.saldo_{tab_ano}_excel') 
    else:
